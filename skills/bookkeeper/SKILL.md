---
name: bookkeeper
description: Categorize a bounded transaction batch against an existing chart of accounts and return a sealed, read-only reconciliation. Use when an operator needs explainable GL mapping, anomaly flags, and an explicit needs-review lane without posting a journal.
registry_owner: hkaaki
---

# Bookkeeper

Use this skill to map a supplied transaction batch onto a supplied chart of
accounts. The result is evidence for a human reviewer. It is not authority to
post a journal entry. The skill performs no ledger mutation, bank action,
payment, file write, network request, or downstream dispatch.
...
4. Categorize only when one account has a unique positive match. Record the
   chart account, confidence, reason, and matched terms.
...
6. Flag dates outside `prior_period.start_date` through `prior_period.end_date`.
   Those anomalies remain read-only observations.
7. Reconcile the batch by reporting matched and unmatched counts. Confirm the
   no-write proof before handing the artifact to an operator.
...
- `chart_of_accounts[]`: objects with unique `code`, `name`, optional `type`,
  and optional `keywords[]`. Keywords are evidence supplied by the caller.
...
- `anomalies[]`: out-of-period or unmatched observations tied to a transaction
  identifier.
- `reconciliation`: `matched`, `unmatched`, and overall `reconciled` or
  `needs_review` status.
- `needs_review`: whether human review is required and why.
- `read_only`: `ledger_mutation: false`, empty `writes`, empty
  `external_effects`, and zero write count.

## Recovery rules

- Add an account to the chart through the caller's normal chart-governance
  process, then rerun. This skill never adds it.
- Resolve a tie by supplying an exact existing `account_code` or by repairing
  the chart keywords. Do not lower the unique-match requirement.
...
```text
runx add hkaaki/bookkeeper@0.1.0
runx skill hkaaki/bookkeeper@0.1.0 --json --input-json transactions='[...]' --input-json chart_of_accounts='[...]' --input-json prior_period='{...}'
